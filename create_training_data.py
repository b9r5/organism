...
with open('data/organism_names.txt', 'r', encoding='utf-8') as f:
    lines = f.readlines()

random.seed(42)

# shuffle the names, since we intend on using the first 90% for training, and we don't want to bias training to any
# structure that might exist in the ordering of the original list (it's unlikely that this matters)
random.shuffle(lines)

with open('data/shuffled_organism_names.txt', 'w') as f:
    for line in lines:
        try:
